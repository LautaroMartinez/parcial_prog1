    ...
    do {
        switch (reportsMenu()) {
            ...
            case 4:
                ...
                break;
            case 5:
                makeflightCodeFile(passengerList);
                break;
            case 6:
                countByPassType(passengerList);
                break;
            case 7:
                initPassengerMiles(passengerList);
                break;
            case 8:
                printf("Volviendo a Menu\n");
                system("pause");
...
int isFirstClass(void* a) {
    return ((Passenger*)a)->passenger_type == 1;
}

int isExecutiveClass(void* a) {
    return ((Passenger*)a)->passenger_type == 2;
}

int isEconomyClass(void* a) {
    return ((Passenger*)a)->passenger_type == 3;
}

int hasFlightCode(void* a, char param[]) {
    return strcmp(((Passenger*)a)->flight_code, param);
}

void calculateMiles(void* a){
    int price_int;
    int miles;
    int multiplier = 1;

    price_int = (int)((Passenger*)a)->price; // sacado de https://stackoverflow.com/questions/24723180/c-convert-floating-point-to-int  
    if (isFirstClass(a)) {
        multiplier = 2;
    } else if (isExecutiveClass(a)) {
        multiplier = 3;
    } 

    miles = price_int*100*multiplier;
    set_miles(a, miles);
}

void countByPassType(LinkedList* passengerList){
    int countFirstClass = 0;
    int countExecutiveClass = 0;
    int countEconomyClass = 0;

    countFirstClass = ll_count(passengerList, isFirstClass);
    countExecutiveClass = ll_count(passengerList, isExecutiveClass);
    countEconomyClass = ll_count(passengerList, isEconomyClass);
    printf("Pasajeros por clase | FirstClass: %d, ExecutiveClass: %d, EconomyClass: %d\n", countFirstClass, countExecutiveClass, countEconomyClass);
}

void makeflightCodeFile(LinkedList* passengerList) {
    LinkedList* fcPassengerList; 
    char flight_code[51];

    getString(flight_code, 10, "Ingrese el Codigo de Vuelo: ", "");
    fcPassengerList = ll_filter(passengerList, hasFlightCode, flight_code);
    saveAsText(fcPassengerList, "data-export.cvs");
    printf("Archivo guardado en data-export.cvs\n");
    system("pause");
}

void initPassengerMiles(LinkedList* passengerList) {
    Passenger* auxPassenger = NULL;
    int len = ll_len(passengerList);
    
    passengerList = ll_map(passengerList, calculateMiles);
    printf("**** Millas por Pasajero: ****\n");
    for(int i = 0; i < len; i++) {
        auxPassenger = ll_get(passengerList, i);
        printf("- %s %s: %i Millas\n", get_name(auxPassenger), get_last_name(auxPassenger), get_miles(auxPassenger));
    }
    printf("**** Fin ****\n");
    system("pause");
}
...
int reportsMenu() {
    ...
    printf("==========Puntos Parcial==========\n");
    printf("5- Generar Archivo de Codigo de Vuelos\n");
    printf("6- Contar por Clase\n");
    printf("7- Calcular Millas\n");
    printf("8- Salir\n");
    printf("Opcion: ");
    fflush(stdin);
    scanf("%d", &opt);
